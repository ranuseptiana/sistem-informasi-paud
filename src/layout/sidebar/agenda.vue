...
<div class="container">
    <section class="content-header">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item active" aria-current="page" style="color: #A9A9A9;">Agenda</li>
            </ol>
        </nav>
        <div class="header-button">
            <h3 class="header-text">Agenda</h3>
            <button class="btn-add-class" @click="prepareTambahAgenda">Tambah Data
            ...
            <table class="table data-table table-hover">
                <thead>
                    <tr>
                        ...
                        <th scope="col" class="table-head">Nama Kegiatan</th>
                        <th scope="col" class="table-head">Kategori Kegiatan</th>
                        <th scope="col" class="table-head">Perkiraan Waktu</th>
                        <th scope="col" class="table-head">Keterangan</th>
                        <th scope="col" class="table-head">Action</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="(agenda, index) in paginatedAgendaList" :key="agenda.id">
                        <td>{{ index + 1 + (currentPage - 1) * rowsPerPage }}</td>
                        <td>{{ agenda.nama_kegiatan }}</td>
                        <td>{{ agenda.kategori_kegiatan }}</td>
                        <td>{{ agenda.perkiraan_waktu }}</td>
                    ...
                    <tr v-if="paginatedAgendaList.length === 0" class="no-data">
            ...
            <div class="custom-modal-dialog">
                <div class="custom-modal-content">
                    <div class="custom-modal-header">
                        <h5 class="custom-modal-title">{{ isEdit ? 'Edit Data Agenda' : 'Tambah Data Agenda' }}</h5>
                        <button type="button" class="close-btn" @click="closeModal">&times;</button>
                    </div>
                    <form @submit.prevent="simpanAgenda">
                        <div class="custom-modal-body">
                            <div class="form-group-agenda">
                                <div class="satu-row">
                                    <label for="kegiatan">Nama Kegiatan:</label>
                                    <input type="text" v-model="form.nama_kegiatan" class="form-input" />

                                    <label for="kategori">Kategori Kegiatan:</label>
                                    <input type="text" v-model="form.kategori_kegiatan" class="form-input" />

                                    <label for="waktu">Perkiraan Waktu:</label>
                                    <input type="text" v-model="form.perkiraan_waktu" class="form-input" />
...
export default {
    ...
    methods: {
        ...
        prepareEditAgenda(id) {
            this.isEdit = true;
            this.tampilModal = true;
            this.dropdownIndex = null;

            const agenda = this.agendaList.find(k => k.id === id);
            if (agenda) {
                this.form = {
                    id: agenda.id,
                    nama_kegiatan: agenda.nama_kegiatan || '',
                    kategori_kegiatan: agenda.kategori_kegiatan || '',
                    perkiraan_waktu: agenda.perkiraan_waktu || '',
        ...
        getFilteredData() {
            return this.agendaList.map((agenda, index) => {
                const filteredAgenda = {
                    No: index + 1
                };
                Object.keys(this.selectedFilters).forEach(key => {
                    if (this.selectedFilters[key]) {
                        filteredAgenda[key] = agenda[key];
                    }
                });
        ...
        simpanAgenda() {
            const url = this.isEdit ?
                `/agenda/${this.form.id}` // route update
                :
                '/agenda'; // route create
            ...
            const payload = {
                ...
                kategori_kegiatan: this.form.kategori_kegiatan,
                perkiraan_waktu: this.form.perkiraan_waktu,
    ...
    computed: {
        filteredAgendaList() {
            ...
            return this.agendaList.filter(agenda => {
                return Object.keys(agenda).some(key => {
                    let value = agenda[key];
        ...
        paginatedAgendaList() {
            ...
            return this.filteredAgendaList.slice(startIndex, endIndex);
        },
        totalPages() {
            return Math.ceil(this.agendaList.length / this.rowsPerPage);
        },
        pageInfo() {
            const startRow = (this.currentPage - 1) * this.rowsPerPage + 1;
            const endRow = Math.min(this.currentPage * this.rowsPerPage, this.agendaList.length);
            return `Showing ${startRow} - ${endRow} of ${this.agendaList.length} entries`;
        },
    },
};
</script>
