<template>
    <div class="main-content">
        ...
        <!-- Daftar Kelas dalam Card -->
        <div class="kelas-container" v-if="!selectedKelas">
            <h2>Daftar Kelas</h2>
            <div class="kelas-cards">
                <div class="kelas-card" v-for="kelas in daftarKelas" :key="kelas.id" @click="selectKelas(kelas)">
                    <div class="card-content">
                        <h3>{{ kelas.nama_kelas }}</h3>
                        <p>Jumlah Siswa: {{ kelas.siswa ? kelas.siswa.length : 0 }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="kotak-peserta" v-else>
            <div class="list-peserta">
                <button class="back-button" @click="selectedKelas = null">
                    <i class="fas fa-arrow-left"></i> Kembali ke Daftar Kelas
                </button>
                <h2>Kelas {{ selectedKelas.nama_kelas }}</h2>
            </div>
    ...
    export default {
        data() {
            return {
                searchQuery: "",
                daftarKelas: [], // Menyimpan semua kelas yang diampu guru
                selectedKelas: null, // Kelas yang dipilih untuk dilihat detailnya
                siswaList: [], // Siswa dari kelas yang dipilih
                username: "",
                password: "",
                currentPage: 1,
                rowsPerPage: 5
        ...
        methods: {
            async fetchKelasByGuruId(idGuru) {
                try {
                    ...
                    this.daftarKelas = response.data.data.daftar_kelas || [];
                    
                    // Jika hanya ada 1 kelas, langsung tampilkan
                    if (this.daftarKelas.length === 1) {
                        this.selectKelas(this.daftarKelas[0]);
                    }
            ...
            selectKelas(kelas) {
                ...
                this.siswaList = kelas.siswa || [];
                this.currentPage = 1; // Reset ke halaman 1 saat ganti kelas
            },
    ...
    <style scoped>
    /* Tambahkan style untuk card kelas */
    .kelas-container {
        width: 94%;
